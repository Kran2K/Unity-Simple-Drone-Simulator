...
이 프로젝트는 실제 드론의 공기 역학이나 모터 물리 등을 정밀하게 계산하는 물리 시뮬레이터가 아닙니다. 대신, 목표 위치/속도에 대한 보간(Interpolation)과 가속도 기반의 **Fake Tilt** 연출을 통해 **"그럴듯한 움직임"을 가볍게 구현**한 기구학적(Kinematic) 시뮬레이터입니다.

**주요 활용 목적:**
*   **Dependency Free**: ROS, Gazebo, PX4와 같은 복잡하고 무거운 기술 스택(설치 및 설정의 어려움) 없이, Unity 실행 파일 하나로 즉시 테스트 환경을 구축합니다.
*   **High-Level Control 검증**: 비행 제어(Flight Control) 레벨이 아닌, 경로 계획, 미션 수행, 군집 제어 등 **상위 레벨의 알고리즘**을 외부 클라이언트에서 개발하고 검증하는 데 최적화되어 있습니다.
*   **Educational Platform**: 드론 시스템의 복잡한 하위 구조(Flight Dynamics, EKF 등)를 추상화하여, 학생들이 **네트워크 통신(UDP), 좌표계 변환, 제어 로직 설계**와 같은 핵심 CS/Engineering 역량 학습에 집중할 수 있는 진입 장벽 낮은 교육 도구로 적합합니다.

## 통신 프로토콜

통신은 **UDP**를 기반으로 하며, 데이터는 **JSON** 형식으로 직렬화됩니다.

-   **명령 수신 (Client -> Unity)**: Unity는 `50100` 포트에서 명령을 수신합니다.
-   **텔레메트리 송신 (Unity -> Client)**: Unity는 `<Client IP>:50200` 주소로 텔레메트리 데이터를 송신합니다.

### 텔레메트리 (Unity -> Client)

시뮬레이터는 드론의 상태 정보를 **20Hz** (`0.05초` 주기)로 클라이언트에 전송합니다.

-   **송신 주소**: `<Client IP>:50200`
...
## 영상 스트리밍
스트리밍은 h264 mpegts 포맷을 기반으로하여 udp 스트림으로 전송합니다.
송출할 IP/PORT는 유니티의 ESC 메뉴에서 설정할 수 있습니다.

기본적으로 드론 아래의 직하방 시점 카메라(아래로90도)를 640x480해상도로 도시하여
해당 영역을 캡처하여 인코딩합니다.

ffmpeg사용하지만, 라이브러리를 직접 참조하여 사용하는것이 아닌
Process를 통하여 외부 프로그램을 실행하고, CLI Argument를 통한 입력으로 파라미터를 적용합니다.
그로인해 C:\ffmpeg\bin\ 경로에 ffmpeg.exe 파일이 존재해야 하며, 환경변수 설정을 필요로 합니다.
ffmpeg 다운로드는 https://github.com/btbn/ffmpeg-builds/releases 에서 하실 수 있습니다.
